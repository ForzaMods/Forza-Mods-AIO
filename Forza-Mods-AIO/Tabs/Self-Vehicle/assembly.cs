...
public class assembly
{
    ...
    #region DLL Imports and its vars
    ...
    public static void GetWayPointXAddr(IntPtr CodeCave4)
    {
        ...
        Array.Reverse(jmpBackBytes);
        string InsideCaveCodeString = "48893D220000000F1097A0030000E9" + BitConverter.ToString(jmpBackBytes).Replace("-", String.Empty).Replace("FFFFFFFF", String.Empty); // move reg to address within code cave + original code + jump back
        if (MainWindow.mw.gvp.Plat == "Forza Horizon 5")
            InsideCaveCodeString = "48893D220000000F109750020000E9" + BitConverter.ToString(jmpBackBytes).Replace("-", String.Empty).Replace("FFFFFFFF", String.Empty);
        byte[] InsideCaveCode = StringToBytes(InsideCaveCodeString);
        ...
        Thread.Sleep(25);
        long WayPointBaseAddr = MainWindow.mw.m.ReadLong((CodeCave4 + 41).ToString("X"));
        ...
        while (WayPointBaseAddr == 0)
        {
            ...
            try
            {
                WayPointBaseAddr = MainWindow.mw.m.ReadLong((CodeCave4 + 41).ToString("X"));
            }
            catch { }
        }
        ...
        MainWindow.mw.m.WriteBytes(Self_Vehicle_Addrs.WayPointxASMAddr, MainWindow.mw.gvp.Name == "Forza Horizon 5" ? new byte[] { 0x0F, 0x10, 0x97, 0x50, 0x02, 0x00, 0x00 } : new byte[] { 0x0F, 0x10, 0xA0, 0x90, 0x03, 0x00, 0x00 });
    }
