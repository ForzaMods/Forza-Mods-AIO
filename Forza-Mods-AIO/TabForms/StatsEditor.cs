            ...
            try
            {
                ...
                else
                {
                    if (Process.GetProcessesByName("ForzaHorizon5")[0].MainModule.FileName.Contains("Microsoft.624F8B84B80"))
                        yeet = (await MainWindow.m.AoBScan(ScanStartAddr, ScanEndAddr, "30 A1 ? ? ? 7F 00 00", true, true)).ToList();
                    else
                        yeet = (await MainWindow.m.AoBScan(ScanStartAddr, ScanEndAddr, "40 18 ? ? ? 7F 00 00", true, true)).ToList();
                }
